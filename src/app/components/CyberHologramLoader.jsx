...
// --- PART 1: THE 3D PARTICLE COMPONENT (Unchanged) ---
...
const CyberHologramLoader = () => {
  ...
  useEffect(() => {
    ...
    let codeInterval = setInterval(() => {
      ...
      setCodeRight(`0x${Math.floor(Math.random() * 16777215).toString(16).toUpperCase()}`);
    }, 150);
  ...
  return (
    <div className={`fixed inset-0 z-9999 flex flex-col font-mono pointer-events-none`}>
      ...
      >
        ...
        <div className="mb-[-50px] z-20 text-center">
            <h1 className="text-[10vw] font-bold text-white leading-none tracking-tighter">
                {displayText}
            </h1>
        </div>
        <div className="absolute top-10 left-10 text-xs text-green-500">
      ...
      >
         <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-10"></div>
         <div className="mt-[-50px] z-20 text-center">
            <h1 className="text-[10vw] font-bold text-white leading-none tracking-tighter opacity-0">
                {displayText} 
            </h1>
            <div className="mt-20 flex flex-col items-center gap-2">
                <div className="h-1 w-64 bg-gray-800">
                    <div className="h-full bg-green-500 transition-all duration-75 ease-linear" style={{ width: `${progress}%` }}></div>
                </div>
                <p className="text-xs text-green-500">{progress}% COMPLETE</p>
            </div>
         </div>
        <div className="absolute bottom-10 right-10 text-xs text-green-500 text-right">
